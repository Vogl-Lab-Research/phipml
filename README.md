...
New to `phipml`? Start with the [**hands-on tutorial**](https://csreynab.github.io/phipml/).

It provides a complete, runnable **data-to-plots walkthrough** using synthetic
examples and shows how to:

- install `phipml` and prepare the input data;
- train and evaluate **Random Forest** and **XGBoost** models;
- configure model YAML files and understand the relevant hyperparameters;
- run nested cross-validation and independent external validation; and
- configure and generate ROC, precision-recall, classification, SHAP, and
  feature-summary plots.

The tutorial includes perfect, noisy, tuned, and external-validation examples,
with ready-to-run configurations and generated reference results.
